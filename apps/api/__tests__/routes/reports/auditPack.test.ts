...
const { default: auditPackRouter } =
  await import("../../../src/routes/reports/auditPack.js");

function makeApp() {
  const app = new Hono<{ Bindings: Env }>();
  app.route("/", auditPackRouter);
...
const auditPackPath ="/reports/audit-pack?communityId=comm-1&periodStart=2024-01-01&periodEnd=2024-12-31";

describe("GET /reports/audit-pack", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await makeRequest(auditPackPath, { method: "GET" }, mockEnv);
  ...
  it("returns 400 when required query params are missing", async () => {
    ...
    const res = await makeRequest("/reports/audit-pack?communityId=comm-1&periodEnd=2024-12-31",
      { method: "GET" },
      mockEnv,
    );
  ...
  it("returns 403 for viewer members before building audit pack", async () => {
    mockGetSession.mockResolvedValueOnce({ user: { id: "user-1" } });
  ...
  it("returns application/zip content with expected filename on happy path", async () => {
    mockGetSession.mockResolvedValueOnce({ user: { id: "user-1" } });
    mockMembership("owner");
    mockTier("price_scale");
    mockBuildAuditPack.mockResolvedValueOnce(new ReadableStream<Uint8Array>());

    const res = await makeRequest(auditPackPath, { method: "GET" }, mockEnv);

    expect(res.status).toBe(200);
    expect(res.headers.get("Content-Type")).toBe("application/zip");
    expect(res.headers.get("Content-Disposition")).toContain("comm-1");
    expect(mockBuildAuditPack).toHaveBeenCalledOnce();
    const [, query] = mockBuildAuditPack.mock.calls[0] as [
      unknown,
      { communityId: string; periodStart: string; periodEnd: string },
    ];
