...
const { default: balanceSheetRouter } =
  await import("../../../src/routes/reports/balanceSheet.js");

function makeApp() {
  const app = new Hono<{ Bindings: Env }>();
  app.route("/", balanceSheetRouter);
...
describe("GET /reports/balance-sheet", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await makeRequest("/reports/balance-sheet?communityId=comm-1&asOf=2024-12-31",
      { method: "GET" },
      mockEnv,
    );
  ...
  it("returns 400 when communityId is missing (zValidator rejects)", async () => {
    ...
    const res = await makeRequest("/reports/balance-sheet?asOf=2024-12-31",
      { method: "GET" },
      mockEnv,
    );
  ...
  it("returns 400 when asOf param is missing", async () => {
    ...
    const res = await makeRequest("/reports/balance-sheet?communityId=comm-1",
      { method: "GET" },
      mockEnv,
    );
  ...
  it("returns 200 with balance sheet on happy path", async () => {
    mockGetSession.mockResolvedValueOnce({ user: { id: "user-1" } });
    ...
    mockBalanceSheet.mockResolvedValueOnce(sampleBalanceSheet);
